...
function update_foto(rep , res){
    let ig=req.params['id'];
    if(req.files.imagen){
        ...
        res.status(500).send({message: '    No se econtró el usuario'})
        ...
        res.status(404).send({message: 'No se coargo ninguna imagen'})
    }
...
if (img !="null") {
     ...
     var path_image= './uploads/perfiles' + img;
     res.status(200).sendFile(path.resolve(path_image))
